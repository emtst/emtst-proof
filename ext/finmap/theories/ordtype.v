...
From mathcomp.ssreflect Require Import ssreflect ssrbool ssrnat eqtype ssrfun seq fintype.
...
Section SumOrd.
Variables L R : ordType.

(* lexicographic ordering of sums, preferring left first *)
Definition sumlex : rel (L + R) :=
  fun x y =>
    match x, y with
    | inl x', inl y' => ord x' y'
    | inr x', inr y' => ord x' y'
...
Lemma irr_sumlex : irreflexive sumlex.
Proof.
  by case ; move=> a ; rewrite /sumlex irr.
Qed.

Lemma trans_sumlex : transitive sumlex.
Proof.
  do !case=>// ;
  do 2!(intro ; case) ;
  do ?apply: trans ; easy.
...
Lemma total_sumlex : forall x y, [|| sumlex x y, x == y | sumlex y x].
Proof.
  do !case=>// ; move=> foo ; case ; do ? apply: total ; easy.
Qed.

Definition sum_ordMixin := OrdMixin irr_sumlex trans_sumlex total_sumlex.
Canonical Structure sum_ordType := Eval hnf in OrdType (L + R) sum_ordMixin.
End SumOrd.
